...
static void rbt_group_decrease_weight(bst *T, bstn *n)
{
    bstn *p = n->parent;
    bstn *pp = p ? p->parent : NULL;
    bstn *l = n->left;
    bstn *r = n->right;
    ...
    /* Case 3: p is red. */
        ...
        bstn *ps = p == pp->left ? pp->right : pp->left;
        assert(ps);
        if (rbtn_color(ps) == BLACK) {
            ...
            if (n == pp->right->right || n == pp->left->left) {
                ...
                if (n == pp->right->right) {
                    bstn_rotate_left(T, pp, NULL);
                } else {
                    bstn_rotate_right(T, pp, NULL);
                }
            ...
            /* Case 3.1b: right-left or left-right ancestor chain. */
                ...
                if (n == pp->right->left) {
                    bstn_rotate_right(T, p, NULL);
                    bstn_rotate_left(T, pp, NULL);
                } else {
                    bstn_rotate_left(T, p, NULL);
                    bstn_rotate_right(T, pp, NULL);
                }
                rbtn_set_color(pp, RED);
                rbtn_set_color(l, BLACK);
...
/* int rbtn_insert(const bst *T, bstn *n, const void *k, bstn **n_out)
 * Insert a node with the key k into the substree rooted at n, preserving the red-black tree
...
int rbtn_insert(
        bst *T,
        bstn *n,         /* the root of the substree, NULL if the tree is empty */
        const void *k,  /* the key to insert */
        bstn **n_out)    /* pass back a pointer to the new node, can be NULL */
{
    ...
    if (n == NULL) {
        ...
        bstn_set_key(T, T->root, k);
        if (n_out) *n_out = T->root;
        return 1;
    }

    int comp = t_compare(T->key_type, k, bstn_key(T, n));
    if (comp == 0) {
        if (n_out) *n_out = n;
    ...
    if (rbtn_color(n) == BLACK) {
        if (comp < 0) {
            ...
            n->left = bstn_new(T);
            assert(!bstn_has_key(n->left));
            bstn_set_key(T, n->left, k);
            n->left->parent = n;
            ...
            n->right = bstn_new(T);
            assert(!bstn_has_key(n->right));
            bstn_set_key(T, n->right, k);
            n->right->parent = n;
    ...
    /* Case 2: n is red */
        ...
        /* Case 2.2: p has 1 red child (n) */
            ...
            if (comp < 0 && n == p->left) {
                ...
                t_move(T->key_type, bstn_key(T, p->left), bstn_key(T, p));
                p->flags.rb.has_key = 0;
                p->left->flags.rb.has_key = 1;
                if (p->flags.rb.has_value) {
                    t_move(T->value_type, bstn_value(T, p->left), bstn_value(T, p));
                ...
                t_move(T->key_type, bstn_key(T, p->right), bstn_key(T, p));
                p->flags.rb.has_key = 0;
                p->right->flags.rb.has_key = 1;
                if (p->flags.rb.has_value) {
                    t_move(T->value_type, bstn_value(T, p->right), bstn_value(T, p));
                    p->flags.rb.has_value = 0;
...
/* int rbt_insert(bst *T, const void *k)
...
/***************************************************************************************
 *
 * static void rbt_group_increase_weight(bst *T, bstn *n)
 *
...
{
    bstn *p = n->parent;
    bstn *s = p ? (n == p->left ? p->right : p->left) : NULL;
    bstn *so = NULL; /* outer child of s */
...
/* int rbtn_remove(const bst *T, bstn *n, const void *k)
 * Remove the node with the key k from the substree roted at n, preserving the red-black tree
        ...
        const void *k)  /* the key to delete */
{
    ...
    for ( ;; ) {
        ...
        comp = t_compare(T->key_type, k, bstn_key(T, n));
    ...
    if (rbtn_color(n) == BLACK) {
        if (n->left) {
            bstn_rotate_right(T, n, NULL);
            rbtn_set_color(n, RED);
            rbtn_set_color(n->parent, BLACK);
        } else if (n->right) {
            bstn_rotate_left(T, n, NULL);
            rbtn_set_color(n, RED);
            rbtn_set_color(n->parent, BLACK);
        }
    }
...
/* int rbt_remove(bst *T, const void *k)
...
/* int rbtn_invariant(const bst *T, const bstn *n)
 * int rbt_invariant(bst *T)
 * Check if T satisfies the inequality properties for keys in binary trees and the red-black tree
 * properties. */

int rbtn_invariant(const bst *T, bstn *n, int *black_count)
{
    if (n) {
        int rc;
        ...
        if (n->parent) {
            if (n == n->parent->left) {
                if (t_compare(T->key_type, bstn_key(T, n), bstn_key(T, n->parent)) > 0) {
